document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderClasses() {
        ...
        classListContainer.innerHTML = classes.map(cls => `
            <button class="class-btn" data-class="${escapeHtml(cls)}" data-tooltip="View ${escapeHtml(cls.replace('_', ' '))} materials" data-tooltip-pos="top">
    ...
    function renderSubjects(subfolders) {
        subjectListContainer.innerHTML = subfolders.map(sub => `
            <button class="subject-pill" data-subject="${escapeHtml(sub)}" data-tooltip="Filter by ${escapeHtml(sub.replace(/_/g, ' '))}" data-tooltip-pos="bottom">${escapeHtml(sub.replace(/_/g, ' '))}</button>
        `).join('');
