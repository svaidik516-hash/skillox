document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderClasses() {
        ...
        classListContainer.innerHTML = classes.map(cls => `
            <button class="class-btn" data-class="${escapeHtml(cls)}" data-tooltip="View ${escapeHtml(cls.replace('_', ' '))} revision notes" data-tooltip-pos="top">
    ...
    function renderCategories(categories) {
        categoryListContainer.innerHTML = categories.map(cat => `
            <button class="subject-pill" data-category="${escapeHtml(cat)}" data-tooltip="Filter by ${escapeHtml(cat.replace(/_/g, ' ').replace(' and ', ' & '))}" data-tooltip-pos="bottom">
    ...
    function renderPDFs(files) {
        if (!files || files.length === 0) {
            pdfGridContainer.innerHTML = `
                <div class="empty-hub" style="grid-column: 1 / -1;">
                    <div style="margin-bottom: 12px; color: var(--primary, #e8740c);">
        ...
        pdfGridContainer.innerHTML = files.map(file => {
            const bookmarkBtn = window.generateBookmarkBtnHtml ? window.generateBookmarkBtnHtml(file.url, file.title) : '';
            return `
            <a href="viewer.html?file=${encodeURIComponent(file.url)}&title=${encodeURIComponent(file.title)}" class="pdf-card" data-tooltip="Open ${escapeHtml(file.title)}">
                ${bookmarkBtn}
                <div class="pdf-icon">
